...
pub fn generate(program: &Program) -> Result<String, CodegenError> {
    ...
    // Add Fast I/O optimization at the start of main
...
fn generate_statement(statement: &Statement, indent_level: usize) -> Result<String, CodegenError> {
    ...
    match statement {
        ...
        Statement::Assignment { target, value } => {
            let target_str = generate_expression(target)?;
...
fn generate_expression(expr: &Expression) -> Result<String, CodegenError> {
    match expr {
        ...
        Expression::ListLiteral(elements) => {
            let elements_str: Result<Vec<String>, CodegenError> = elements.iter()
        ...
        Expression::Call(name, args) => {
            ...
            match name.as_str() {
                ...
                "len" => Ok(format!("rl::len({})", args_str.join(", "))),
                "append" => Ok(format!("rl::append({})", args_str.join(", "))),
                _ => Ok(format!("{}({})", name, args_str.join(", "))),
            }
        },
        Expression::Index { list, index } => {
            let list_str = generate_expression(list)?;
            let index_str = generate_expression(index)?;
            Ok(format!("{}[{}]", list_str, index_str))
        },
